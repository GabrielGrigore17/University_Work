...
public:
    ...
    double operator~(){ return this->module();}
    void operator^(const int &power){
        if(!flag)
            cout << pow(real_number, power) << endl;
        if(flag){
            cout<< pow(complex_number.real_part, 2) - pow(complex_number.imaginary_part, 2) << " + ";
            cout<< 2 * complex_number.real_part * complex_number.imaginary_part << "*i" << endl;
        }
    }
};
...
int main (){
    ...
    cout << ~operation_1 << endl;
    cout << ~operation_2 << endl;
    operation_1 ^ 2;
    operation_2 ^ 2;
